...
export default class Directory extends mixins(JhiDataUtils) {
  ...
  public retrieveAllDirectorys(): void {
        ...
        res => {
          if (res.data && res.data.length > 0) {
            for (let i = 0; i < res.data.length; i++) {
              this.directories.push(res.data[i]);
            }
  ...
  public prepareRemove(instance: IDirectory): void {
  ...
  public removeDirectory(): void {
    this.directoryService()
      .delete(this.removeId)
      .then(() => {
        const message = 'A Directory is deleted with identifier ' + this.removeId;
